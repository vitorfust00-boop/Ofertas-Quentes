...
document.getElementById('btn-submit-market').addEventListener('click', async () => {
    ...
    try {
        ...
        if (db) {
            // Dispara e esquece (sem await) para ser super rápido na tela
            db.collection("markets").add(marketData).catch(err => console.error("Erro background market:", err));
        }

        // Criar o pino visualmente no mapa imediatamente
