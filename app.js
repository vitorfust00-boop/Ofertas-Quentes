...
function initializeLeafletMap(lat, lon) {
    ...
    // SÓ AGORA que o mapa existe, puxamos os mercados do banco!
    loadMarkets();
}
            ...
            if (change.type === "added" || change.type === "modified") {
                if (data.lat && data.lon) {
                    ...
                    if (map) {
                        ...
                        const estaVisivel = map.getBounds().contains([data.lat, data.lon]);
                        console.log(`[ÁREA VISÍVEL] O mercado ${data.nome} está dentro da área visível agora? ${estaVisivel ? 'SIM' : 'NÃO (está em outra cidade/bairro)'}`);
                    } else {
